...
public class ProdutoControle {
		...
		telaProdutos.getBtnCadastrarProd().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (produtoSelecionado) {
					...
					// EditProduto(produto); oq q essa linha de código faz ????????
					// atualizarTabela();
	...
	public void setTelaDeletarProduto(TelaDeletarProduto telaDeletarProduto) {
		this.telaDeletarProduto = telaDeletarProduto;

		telaDeletarProduto.getBtnConfirmar().addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				excluirProduto();
			}
		...
		telaDeletarProduto.getBtnCancelar().addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				telaDeletarProduto.dispose();
			}
		});
 ...
 * table.getSelectedRow(); if (selectedRowIndex == -1) {
 * JOptionPane.showMessageDialog(telaProdutos, "Nenhum produto selecionado",
 * "Erro", JOptionPane.ERROR_MESSAGE); } else {
 * telaCadastroProduto.setVisible(true);
 * telaCadastroProduto.getbtnCadastrarProduto().setText("Editar Produto");
 * telaCadastroProduto.setTitle("Editar Produto"); String firstColumnValue =
 * table.getValueAt(selectedRowIndex, 0).toString(); Produto produto = new
 * Produto(); produto = produtoDAO.getId(Integer.valueOf(firstColumnValue));
 * telaCadastroProduto.setVisible(true); fillEdit(produto); } } }); } //
 * ---------------------------------------Deletar-------------------------------
 * ------
 * 
 ...
 * telaProdutos.getBtnDeletarProd().addActionListener(new ActionListener() {
 * 
 * @Override public void actionPerformed(ActionEvent e) { DeletProduto();
 * listarProdutosTable(); } }); }
 * 
 * //
 * -----------------------------------------------------------------------------
 * ------- public void setCancelarProduto(TelaCadastroProdutos
 * telaCadastroProduto) { this.telaCadastroProduto = telaCadastroProduto;
 * telaCadastroProduto.getbtnCancelar().addActionListener(new ActionListener() {
 * 
 * @Override public void actionPerformed(ActionEvent e) {
 ...
 * produto.getQuantEstoque(), }); } }
 * 
 * public Produto getProdutoById(int id) { Produto produto =
 * produtoDAO.getId(id); if (produto == null) { throw new
 * IllegalStateException("Produt com ID " + id + "não encontrado."); } return
 * produto; }
 * 
 * public void EditProduto(Produto produto) { try { float valor =
 ...
 * telaCadastroProduto.getTextFieldNome(); String material =
 * telaCadastroProduto.getCbMaterial(); String categoria =
 * telaCadastroProduto.getCbCategoria(); String variacao =
 * telaCadastroProduto.getTextFieldVariacao(); String tamanho =
 * telaCadastroProduto.getCbTamnho();
 * 
 ...
 * produto.setCategoria(categoria); produto.setVariacao(variacao);
 * produto.setValor(valor); produto.setQuantEstoque(estoque);
 ...
 * 
 * public void cadastrarProduto(TelaCadastroProdutos cadastroProduto) { // TODO
 * Auto-generated method stub // if quase mais longo que a sua mãe. if
 ...
 * JOptionPane.ERROR_MESSAGE); } }
 * 
 * public void DeletProduto() { // TODO Auto-generated method stub JTable table
